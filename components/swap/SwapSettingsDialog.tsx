...
export function SwapSettingsDialog({ open, settings, onChange, onClose }: SwapSettingsDialogProps) {
  ...
  return (
    <AppDialog open={open} onOpenChange={onClose}>
      <AppDialogContent title="Swap Settings" size="md">
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              {presetSlippages.map((preset) => (
                <AppButton
                  key={preset.label}
                  variant={
                    localSettings.slippageBps === preset.bps && !customSlippage
                      ? "primary"
                      : "ghost"
                  }
                  size="sm"
            ...
            <div className="flex items-center gap-2">
              <AppInput
                placeholder="Custom"
                value={
                  customSlippage || (!isPresetSlippage ? currentSlippagePercent.toString() : "")
                }
          ...
          <div className="flex items-center justify-between">
            ...
            <AppSwitch
              checked={localSettings.oneClickEnabled}
              onCheckedChange={handleOneClick}
              variant="primary"
            />
          </div>

          {/* Save Button */}
          <AppButton variant="primary" className="w-full" onClick={handleSave}>
            Save Settings
          </AppButton>
        </div>
      </AppDialogContent>
